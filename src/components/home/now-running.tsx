...
type State =
  | { status: 'loading' }
  | { status: 'success'; data: NowRunningData }
  | { status: 'error' }

function RunningCard({ state }: { state: State }) {
  ...
  const { name, date, sport } = state.data

  return (
    <div className={wrapper}>
      <div className="flex flex-wrap items-center gap-x-2 text-sm">
        <span>{name}</span>
        <span className="text-muted-foreground/80">{sport}</span>
      </div>
...
export default function NowRunning() {
  ...
  useEffect(() => {
    ...
    async function load() {
      try {
        const response = await fetch('/api/now-running')
        if (!response.ok) throw new Error(`HTTP ${response.status}`)
        const data: NowRunningData = await response.json()
